<template>
  <div class="container my-4" v-if="isHere">
    <worker-template :worker="director"></worker-template>
    <div class="d-flex my-5">
      <div class="left" v-if="director.movies.length">
        <h2 class='my-4'>Movies</h2>
        <slide-watchlist :watchlist="director.movies" idSpec="movies"></slide-watchlist>
      </div>
      <div class="right mx-3" v-if="director.series.length">
        <h2 class='my-4'>Series</h2>
        <slide-watchlist :watchlist="director.series" id-spec="series"></slide-watchlist>
      </div>
  ...
  <div class="container my-5  alert alert-warning d-flex align-items-center" role="alert" v-else>
    ...
    <div class="mx-3">
      Director not found for given credentials!
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    isHere() {
      if (!this.director)
        return false
      return this.director.last_name ? true : false
    }
  },
  data() {
    return {
  ...
  methods:{
    ...
    if(parseInt(currentId))
      api(`show-worker/director/detail/${parseInt(currentId)}`, 'get').then(res => {
